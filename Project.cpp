...
#define MX 1001     //constants for problem 23
#define SZ 1000001
...
void Project::names_score()
{
    ...
    while (pass < 40) {
        ...
        for (n = 0; (n+gap) < max; n++) {
            if (names[n] > names[n+gap])
                swap(names[n], names[n+gap]);
        }
        pass++;
    }
    int z;
    ...
    cout << "The total name scores of the text is "<<total <<endl;
}
...
closest = ( (log(10)*nod) + (log(5)/2) )/log(phi);
n = closest;

cout <<"The first term in the Fibonacci sequence to contain 1000 digits is: "<< n <<endl;
}

void Project:: trunctable_primes()
{
 long number,total = 0;
 sieve();
 for(number = 4;number < nPrime; number++){
  if( isTruncatable(primeTable[number]) ){
   total += primeTable[number];
  }
 }
 cout<<"The total of 11 trunctable primes is: "<<total<<endl;
}
